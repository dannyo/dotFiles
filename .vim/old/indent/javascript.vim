...
function! GetJsIndent()
  ...
  let pnum = s:GetNonCommentLine(lnum-1)
  ...
  " =============================
  if s:IsObjectEnd(line) && !s:IsComment(lnum)
    call s:Log("Line matched object end")

    let obeg = s:GetObjectBeg(lnum)
  ...
  " ============================
  if s:IsArrayEnd(line) && !s:IsComment(lnum)
    call s:Log("Line matched array end")

    let abeg = s:GetArrayBeg(lnum)
  ...
  " Handle: Parens
  " ==============
  if s:IsParenEnd(line) && !s:IsComment(lnum)
    call s:Log("Line matched paren end")

    let abeg = s:GetParenBeg(lnum)
    let aind = indent(abeg)
